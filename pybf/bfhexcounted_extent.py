"""Experimental N-counted hex reader with a compact count-extent invariant.

The established counted reader determines whether the decremented 64-bit count
is nonzero by rescanning all sixteen nibbles after every input element.  This
variant carries one extra byte: the index+1 of the most-significant nonzero
count nibble.  Decrement changes that extent only when the count crosses a
power-of-16 boundary, and the next loop marker is obtained while destructively
moving the small extent value to the next record.

The data/sum ABI and input-line semantics are otherwise identical to
``bfhexcounted``.  This module stays experimental until correctness and runtime
telemetry beat the established reader.
"""
...
def _set_count_extent_and_marker(
    ...
    *,
    ...
    """Initialize extent=max(nonzero nibble index)+1 and MARKER=(count != 0)."""
...
def _decrement_count_with_extent(r: _RelativeBuilder, word_base: int) -> None:
    """Subtract one from a positive count and maintain COUNT_EXTENT.

    If borrow resolves on nibble i by changing 1 -> 0, extent drops exactly
    when that nibble was the most-significant nonzero nibble.  Lower borrowed
    nibbles have become 15, so the new extent is simply old_extent-1.
    """
    borrow = LEFT
    is_zero = LEFT + 1
    tmp = LEFT + 2
    ...
    for i in range(HEX_DIGITS):
        ...
        # After the zero case is repaired to 15, result==0 means precisely
        # that the original nibble was one and borrow resolved here.
...
def _move_extent_and_set_next_marker(r: _RelativeBuilder) -> None:
    """Move the 0..16 extent right and derive the next loop marker for free."""
...
def _counted_record_body() -> str:
    ...
    # LEFT[0] carries the already-ended-line flag. COUNT_EXTENT occupies only
    # LEFT[15], leaving the existing parser/control scratch contract intact.
    r.clear(CH)
...
def read_counted_two_line_s64s_and_sum(
    ...
    """Read N and N participating integers using carried count extent metadata."""
    if seq.base < 0:
