"""Partition prototype carrying D = total - 2*prefix as the only live word."""
...
from bfhexdifference import update_difference_and_duplicate_candidate
...
def _partition_body() -> str:
    r = _RelativeBuilder()
    update_difference_and_duplicate_candidate(r)
...
def run_partition_min_pass(
    ...
    *,
    ...
    """Run the partition recurrence with a single carried difference word."""
    if not 0 <= initial_ans < (1 << 63):
        raise ValueError("difference-state partition requires signed nonnegative initial_ans")
