"""Zero-aware partition variant using DATA->LEFT candidate merging."""
...
from bfhexaddcandidate_datamerge import add_data_and_move_state_total_minus_double_left_into_total
...
def partition_body(extent: int) -> str:
    if not 1 <= extent <= MAX_BOUNDED_NIBBLES:
        raise ValueError("data-merge extent outside supported scratch layout")
...
def run_partition_min_pass(
    ...
    *,
    ...
    if extent > MAX_BOUNDED_NIBBLES:
        raise ValueError("initial_ans is too wide for data-merge prototype")
