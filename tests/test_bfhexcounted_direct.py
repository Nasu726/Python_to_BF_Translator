...
def test_counted_direct_crosses_radix16_count_boundary():
    for n in (15, 16, 17):
        code, seq = _program()
        values = [1] * n
        input_data = f"{n}\n" + " ".join(map(str, values)) + "\n"
...
def test_counted_direct_signed_int64_and_total():
...
def test_counted_direct_honors_n_and_drains_extra_tokens():
    code, seq = _program()
    input_data = "3\n5 -2 10 999 1000\n"
    result = run_bf(code, input_data, memory_size=20_000, step_limit=1_000_000_000)
    assert result.input_consumed == len(input_data)
...
def test_counted_direct_zero_fills_short_line_without_crossing():
    code, seq = _program()
    input_data = "5\n7 -3\n999 1000\n"
...
def test_counted_direct_empty_ranges_drain_second_line():
    for n in (0, -2):
        code, seq = _program()
        input_data = f"{n}\n10 20 30\n"
