...
def test_literal_length_and_dynamic_index():
    ...
    assert signed(result.memory, out.base) == -2
    assert signed(result.memory, length.base) == 3
...
def test_append_uses_original_length_once():
    ...
    assert signed(result.memory, values.item(0).base) == 1
    assert signed(result.memory, values.item(1).base) == 7
    assert signed(result.memory, values.item(2).base) == 0
...
def test_read_list_line_and_drain_overflow():
    ...
    assert [signed(result.memory, first.item(i).base) for i in range(3)] == [1, 2, 3]
    assert result.memory[second.length_cell] == 2
    assert [signed(result.memory, second.item(i).base) for i in range(2)] == [-7, 8]
